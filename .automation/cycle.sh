#!/usr/bin/env bash
# One iteration of the autoreason loop. Called by systemd timer every 15 min.
set -euo pipefail
...
echo "$ISSUES_JSON" | jq -c '.[]' | while read -r row; do
  ...
  case "$CURRENT" in
    ready|pr_opened|blocked|skipped|closed)
      # ready → worker step below handles it
      # pr_opened → PR comment loop handles it
      # blocked/skipped/closed → leave alone unless the issue was edited
      if [[ "$CURRENT" != "ready" && "$CURRENT" != "pr_opened" && "$UPDATED" != "$LAST_UPDATED" ]]; then
...
gh pr list --state open --limit 30 --json number,headRefName,isDraft \
